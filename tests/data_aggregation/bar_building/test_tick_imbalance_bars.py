import numpy as np
import pandas as pd
from quantreo.data_aggregation.bar_building.tick_imbalance_bars import ticks_to_tick_imbalance_bars


def test_ticks_to_tick_imbalance_bars(ticks_sample):
    """Test the ticks_to_tick_imbalance_bars function."""
    df = ticks_sample.copy()

    # === Basic functional call ===
    bars = ticks_to_tick_imbalance_bars(df, expected_imbalance=10)
    ...
    # The number of ticks per bar must also be positive
    ...
    df_original = df.copy()
    ticks_to_tick_imbalance_bars(df_original, expected_imbalance=100)
